...
function VideoAdd({
  ...
  >('downloading');
  // Per-video step-result lines (metadata / transcript / entities) and the
  // live Whisper bar — the same FULL Python-style display the playlist path
  // shows (video_extractor.py:118-120,160-161,202-205). Built from this
  // command's manually-driven pipeline rather than VideoExtractor events.
  const [stepLines, setStepLines] = useState<readonly string[]>([]);
  const [whisperProgress, setWhisperProgress] = useState<
    { stage: 'downloading' | 'transcribing' | 'complete'; percentage?: number } | undefined
  >(undefined);
  ...
  useEffect(() => {
    async function extractVideo() {
      ...
      try {
        ...
        // Metadata step line: Channel · duration (Python video_extractor.py:118-120).
        setStepLines((prev) => [
          ...prev,
          formatMetaLine(details.channelTitle, details.durationSeconds),
        ]);
        ...
        if (!flags.noTranscript) {
          ...
          // Construct the WhisperExtractor with a constructor-level onProgress
          // so the live download/transcription percentage surfaces even though
          // the fallback fires INSIDE TranscriptExtractor.extract (which does
          // not forward a per-call callback). WhisperExtractor uses its
          // constructor `onProgress` for every emit (WhisperExtractor.ts:244).
          const whisper = !flags.noWhisper
            ? new WhisperExtractor({
                onProgress: (progress) => {
                  setWhisperProgress(
                    progress.stage === 'complete'
                      ? undefined
                      : { stage: progress.stage, percentage: progress.percentage }
                  );
                },
              })
            : undefined;
          ...
          // Whisper is done either way once extract returns; clear the bar.
          setWhisperProgress(undefined);
          if (transcript) {
            ...
            // Transcript source + char count (Python video_extractor.py:184).
            const source = transcript.from_whisper ? 'whisper' : 'youtube';
            setStepLines((prev) => [
              ...prev,
              formatTranscriptLine(source, transcript.full_text.length),
            ]);
          } else {
            // No transcript available (Python video_extractor.py:189).
            setStepLines((prev) => [...prev, formatTranscriptLine('none', 0)]);
          }
        }
        ...
        if (!flags.noLlm) {
          ...
          // Entity counts (Python video_extractor.py:160-161). The single-video
          // path parses the description only (no Gemini here), so topics/people
          // are 0 — the line still reports repos/websites when present.
          const entitiesLine = formatEntitiesLine(
          ...
          if (entitiesLine) {
            setStepLines((prev) => [...prev, entitiesLine]);
          }
        }
  ...
  if (status === 'error') {
    return (
      <ErrorDisplay
        message={error || 'Video extraction failed'}
        code={errorInfo?.code}
  ...
  if (status === 'done') {
    return (
      <Box flexDirection="column">
        ...
        <Box marginBottom={stepLines.length > 0 ? 1 : 0}>
          <Text>
            Title: <Text>{videoTitle || 'Unknown'}</Text>
          </Text>
        </Box>
        {/* The per-video result lines persist on the done view, mirroring how
            Python's [OK] lines stay in the console scrollback after extraction
            (video_extractor.py:118-228). */}
        {stepLines.length > 0 && (
          <Box flexDirection="column">
