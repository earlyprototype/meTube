...
*   [ ] [Tier 4] Move the 11 root `.bat` files + 4 root ad-hoc `.ts` scripts into `scripts/dev/` or delete obsolete ones
...
*   [ ] [Tier 1] Port `${VAR}` recursive YAML substitution from `legacy/python/src/cli.py:184-208` into `src-ts/config.ts` (~30 min)
*   [ ] [Tier 1] Add `safeTitle()` terminal-safe Unicode helper in `src-ts/utils/terminal.ts`; apply at every `<Text>{video.title}</Text>` call site (~1 h, see `legacy/python/src/cli.py:561`)
*   [ ] [Tier 1] Add GitHub repo description enrichment to `src-ts/reports/HTMLReportGenerator.ts:aggregatePlaylistData` — port `_fetch_github_description` from `legacy/python/src/reports/html_generator.py:228-265` (~45 min)
*   [ ] [Tier 1] Rewrite README: lead with ADHD-tool framing, embed CLI screencast/GIF, drop Python-version setup steps (~1 h)
*   [ ] [Tier 1] Honest one-paragraph status banner in README — name what's untested (post-extraction menu, playlist videos, video add, HTML reports) (~15 min)
